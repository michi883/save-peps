...
> **Superseded, 21 Aug 2026 — environment reuse.** The two bullets above are the reuse plan that got the catalogue built, and it worked: 36 rescues shipped on schedule. It also produced the failure it was always going to produce — with 14 dioramas shared across 12 rounds, a "Canyon" round contained two garden scenes, and the rounds read as one toy puzzle with twelve skins rather than twelve places. The revamp replaced it with **12 worlds × 3 stages = 36 stages, one per rescue**, and **36 props** in which the world-native singletons (gear, thruster, chime crystal, zip grip …) sit alongside the protean core that still travels. Reuse now lives at the level of *systems* — world kits, the prop library, the choreography vocabulary — rather than at the level of geometry. `ContentValidator` enforces it; `design/ROUND_CATALOG.md` documents it. The throughput argument below still stands, because the world kits made a stage cheaper to build than a bespoke diorama was.
